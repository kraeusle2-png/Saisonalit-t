...
// ─── DATA ────────────────────────────────────────────────────────────────────
const TRADES = [
  { id:1,  name:"S&P 500 / DAX",  direction:"LONG",  startMonth:11, endMonth:4,  baseHitRate:75, avgGain:"+7%",      avgGainPct:7,  windowDays:181, icon:"📈", sector:"Aktien",    fundamentalDriver:"Best Six Months – institutionelle Portfolioanpassungen & Jahresendrallye", stopLoss:9,  volatility:"mittel" },
  { id:2,  name:"Erdgas",          direction:"LONG",  startMonth:9,  endMonth:10, baseHitRate:77, avgGain:"hoch",     avgGainPct:15, windowDays:61,  icon:"🔥", sector:"Rohstoff",  fundamentalDriver:"Vorheizperiode, Lagerbefüllung vor Winter", stopLoss:18, volatility:"sehr hoch" },
  { id:3,  name:"Gold / Silber",   direction:"LONG",  startMonth:1,  endMonth:2,  baseHitRate:78, avgGain:"+6%",      avgGainPct:6,  windowDays:59,  icon:"🥇", sector:"Edelmetall", fundamentalDriver:"Schmucknachfrage Asien, Jahresbeginn-Käufe", stopLoss:9, volatility:"mittel" },
  { id:4,  name:"Rohöl / Benzin",  direction:"LONG",  startMonth:2,  endMonth:5,  baseHitRate:70, avgGain:"+10%",     avgGainPct:10, windowDays:120, icon:"🛢️", sector:"Rohstoff",  fundamentalDriver:"Frühjahrs-Reisesaison, Raffinerie-Umstellungen", stopLoss:13, volatility:"hoch" },
  { id:5,  name:"S&P 500 Q4",      direction:"LONG",  startMonth:10, endMonth:11, baseHitRate:80, avgGain:"+4%",      avgGainPct:4,  windowDays:61,  icon:"🎄", sector:"Aktien",    fundamentalDriver:"Weihnachtsrallye, Q4-Portfolioanpassungen", stopLoss:9, volatility:"mittel" },
  { id:6,  name:"GBP/JPY",         direction:"SHORT", startMonth:7,  endMonth:8,  baseHitRate:80, avgGain:"450 Pips", avgGainPct:3,  windowDays:62,  icon:"💴", sector:"Forex",     fundamentalDriver:"Risk-off Spätsommer, JPY-Stärke als sicherer Hafen", stopLoss:10, volatility:"hoch" },
  { id:7,  name:"AUD/JPY",         direction:"SHORT", startMonth:7,  endMonth:8,  baseHitRate:80, avgGain:"241 Pips", avgGainPct:3,  windowDays:62,  icon:"🦘", sector:"Forex",     fundamentalDriver:"Risk-off Dynamik, JPY sicherer Hafen", stopLoss:10, volatility:"hoch" },
  { id:8,  name:"Zucker",          direction:"SHORT", startMonth:6,  endMonth:8,  baseHitRate:72, avgGain:"mittel",   avgGainPct:6,  windowDays:92,  icon:"🍬", sector:"Rohstoff",  fundamentalDriver:"Erntedruck, Angebotsanstieg Südhalbkugel", stopLoss:13, volatility:"hoch" },
  { id:9,  name:"Kupfer",          direction:"LONG",  startMonth:2,  endMonth:4,  baseHitRate:70, avgGain:"+6%",      avgGainPct:6,  windowDays:90,  icon:"🔶", sector:"Metall",    fundamentalDriver:"Frühjahr-Bausaison, China-Nachfrage", stopLoss:9, volatility:"mittel" },
  { id:10, name:"Nasdaq / Tech",   direction:"LONG",  startMonth:10, endMonth:1,  baseHitRate:75, avgGain:"+10%",     avgGainPct:10, windowDays:92,  icon:"💻", sector:"Aktien",    fundamentalDriver:"Jahresendrallye + Januar-Effekt", stopLoss:11, volatility:"hoch" },
];
...
// ─── LATE ENTRY CHECKER COMPONENT ────────────────────────────────────────────
function LateEntryChecker({ trades, activeMonth, months, liveData }) {
  const [selectedTrade, setSelectedTrade] = useState(null);
  const [alreadyMoved, setAlreadyMoved]   = useState("");
  const [daysPassed, setDaysPassed]       = useState("");
  const [signalType, setSignalType]       = useState("none");

  const activeTrades = trades.filter(t => isTradeActive(t, activeMonth));
  const trade = trades.find(t => t.id === selectedTrade);

  function getWindowDaysPassed(trade, currentMonth) {
    // Calculate how far into the window we are (approximate by months)
    const { startMonth, endMonth, windowDays } = trade;
    const monthsInWindow = endMonth >= startMonth
      ? endMonth - startMonth + 1
      : (12 - startMonth) + endMonth + 1;
    let monthsElapsed = endMonth >= startMonth
      ? currentMonth - startMonth
      : currentMonth >= startMonth ? currentMonth - startMonth : (12 - startMonth) + currentMonth;
    return Math.round((monthsElapsed / monthsInWindow) * windowDays);
  }

  function evaluate() {
    if (!trade) return null;
    const moved    = parseFloat(alreadyMoved) || 0;
    const days     = parseFloat(daysPassed)   || (trade ? getWindowDaysPassed(trade, activeMonth) : 0);
    const windowPct = days / trade.windowDays;
    const gainUsed  = moved / trade.avgGainPct;
    const remaining = trade.avgGainPct - moved;
    const daysLeft  = trade.windowDays - days;
    const hasSignal = signalType !== "none";

    // Three criteria
    const c1_window  = windowPct <= 0.5;   // Less than 50% of window elapsed
    const c2_gain    = gainUsed  <= 0.5;   // Less than 50% of avg gain already made
    const c3_signal  = hasSignal;          // Has a valid entry signal

    const passedCount = [c1_window, c2_gain, c3_signal].filter(Boolean).length;

    let verdict, color, bg, advice, posSize;
    if (passedCount === 3) {
      verdict  = "✅ Späteinstieg empfohlen";
      color    = T.green; bg = T.greenLight;
      posSize  = "75%";
      advice   = `Alle drei Kriterien erfüllt. Steige mit 75% der normalen Positionsgröße ein (reduziert wegen späterem Einstieg). Stop-Loss unverändert bei –${trade.stopLoss}%. Noch ca. ${daysLeft} Tage im Fenster und ~${remaining.toFixed(1)}% Restpotenzial.`;
    } else if (passedCount === 2 && c3_signal) {
      verdict  = "🟡 Möglich – aber Vorsicht";
      color    = T.yellow; bg = T.yellowLight;
      posSize  = "50%";
      advice   = `Zwei Kriterien erfüllt. Wenn überhaupt, dann nur mit 50% der normalen Positionsgröße einsteigen. Das Restpotenzial ist begrenzt. Nur bei sehr starkem Signal.`;
    } else if (passedCount === 2 && !c3_signal) {
      verdict  = "🟠 Warten auf Signal";
      color    = T.orange; bg = T.orangeLight;
      posSize  = "–";
      advice   = `Die zeitlichen Kriterien stimmen noch, aber es fehlt das technische Einstiegssignal (Pullback oder Ausbruch). Noch nicht einsteigen – warte auf eine klare Kursbewegung.`;
    } else {
      verdict  = "❌ Nicht einsteigen";
      color    = T.red; bg = T.redLight;
      posSize  = "–";
      advice   = `Zu viel des saisonalen Fensters oder zu viel des durchschnittlichen Gewinns ist bereits verstrichen. Das Chance-Risiko-Verhältnis lohnt nicht mehr. Warte auf das nächste saisonale Fenster.`;
    }

    return { c1_window, c2_gain, c3_signal, passedCount, verdict, color, bg, advice, posSize, windowPct, gainUsed, daysLeft, remaining, days };
  }

  const result = selectedTrade ? evaluate() : null;

  return (
    <>
      <SectionLabel>▶ Späteinstieg-Checker</SectionLabel>
      <div style={{ background:T.blueLight, border:`1px solid ${T.blue}33`, borderRadius:12, padding:"14px 16px", marginBottom:18, fontSize:13, color:"#1e3a5f", lineHeight:1.7 }}>
        <strong>Wann lohnt sich der Einstieg noch?</strong> Dieser Checker prüft die drei entscheidenden Kriterien für einen späten Einstieg in ein bereits laufendes saisonales Fenster.
      </div>

      <Card style={{ marginBottom:14 }}>
        <SectionLabel>Schritt 1 – Instrument wählen</SectionLabel>
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
          {trades.filter(t => isTradeActive(t, activeMonth)).map(t => (
            <button key={t.id} onClick={() => { setSelectedTrade(t.id); setAlreadyMoved(""); setDaysPassed(""); setSignalType("none"); }} style={{
              display:"flex", alignItems:"center", gap:10, padding:"12px 14px", borderRadius:10, cursor:"pointer", textAlign:"left",
              background: selectedTrade===t.id ? T.blueLight : T.surfaceAlt,
              border: `2px solid ${selectedTrade===t.id ? T.blue : T.border}`,
            }}>
              <span style={{ fontSize:20 }}>{t.icon}</span>
              <div>
                <div style={{ fontSize:13, fontWeight:700, color:T.text }}>{t.name}</div>
                <div style={{ fontSize:11, color:T.textSub }}>Ø {t.avgGain} · {t.windowDays} Tage Fenster</div>
              </div>
            </button>
          ))}
          {activeTrades.length === 0 && (
            <div style={{ gridColumn:"1/-1", padding:20, textAlign:"center", color:T.textMuted, border:`1px dashed ${T.border}`, borderRadius:10, fontSize:13 }}>
              Keine aktiven Trades in diesem Monat – Monat oben anpassen.
            </div>
          )}
        </div>
      </Card>
      ...
      {trade && (
        <Card style={{ marginBottom:14 }}>
          <SectionLabel>Schritt 2 – Aktuelle Situation eingeben</SectionLabel>

          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
            <div>
              <div style={{ fontSize:12, fontWeight:700, color:T.textSub, marginBottom:6 }}>
                UM WIEVIEL % HAT SICH DAS ASSET BEREITS BEWEGT?
              </div>
              <div style={{ display:"flex", gap:6, marginBottom:6 }}>
                {[1,2,3,4,5,6,8].map(v => (
                  <button key={v} onClick={() => setAlreadyMoved(v.toString())} style={{
                    flex:1, padding:"7px 2px", borderRadius:8, cursor:"pointer", fontSize:12, fontWeight:700,
                    background: alreadyMoved===v.toString() ? T.blue : T.surfaceAlt,
                    border: `1px solid ${alreadyMoved===v.toString() ? T.blue : T.border}`,
                    color: alreadyMoved===v.toString() ? "#fff" : T.textSub,
                  }}>{v}%</button>
                ))}
              </div>
              <input type="number" value={alreadyMoved} onChange={e => setAlreadyMoved(e.target.value)}
                placeholder="oder eigenen Wert eingeben..."
                style={{ width:"100%", background:T.surfaceAlt, color:T.text, border:`1px solid ${T.border}`, borderRadius:9, padding:"9px 11px", fontSize:13, boxSizing:"border-box" }} />
              <div style={{ fontSize:11, color:T.textMuted, marginTop:4 }}>Ø historischer Gewinn: {trade.avgGainPct}%</div>
            </div>

            <div>
              <div style={{ fontSize:12, fontWeight:700, color:T.textSub, marginBottom:6 }}>
                WIEVIELE TAGE IST DAS FENSTER BEREITS OFFEN?
              </div>
              <div style={{ display:"flex", gap:6, marginBottom:6 }}>
                {[7,14,21,30].map(v => (
                  <button key={v} onClick={() => setDaysPassed(v.toString())} style={{
                    flex:1, padding:"7px 2px", borderRadius:8, cursor:"pointer", fontSize:12, fontWeight:700,
                    background: daysPassed===v.toString() ? T.blue : T.surfaceAlt,
                    border: `1px solid ${daysPassed===v.toString() ? T.blue : T.border}`,
                    color: daysPassed===v.toString() ? "#fff" : T.textSub,
                  }}>{v}d</button>
                ))}
              </div>
              <input type="number" value={daysPassed} onChange={e => setDaysPassed(e.target.value)}
                placeholder={`oder eingeben (Fenster: ${trade.windowDays} Tage)`}
                style={{ width:"100%", background:T.surfaceAlt, color:T.text, border:`1px solid ${T.border}`, borderRadius:9, padding:"9px 11px", fontSize:13, boxSizing:"border-box" }} />
              <div style={{ fontSize:11, color:T.textMuted, marginTop:4 }}>Gesamtes Fenster: {trade.windowDays} Tage</div>
            </div>
          </div>

          <div style={{ marginTop:14 }}>
            <div style={{ fontSize:12, fontWeight:700, color:T.textSub, marginBottom:8 }}>GIBT ES EIN TECHNISCHES EINSTIEGSSIGNAL?</div>
            <div style={{ display:"flex", gap:8 }}>
              {[
                { id:"none",     label:"Kein Signal",              desc:"Noch kein klares Muster" },
                { id:"pullback", label:"📉 Pullback",              desc:"Rücksetzer auf MA oder Support" },
                { id:"breakout", label:"📈 Ausbruch",              desc:"Ausbruch aus Konsolidierung" },
              ].map(s => (
                <button key={s.id} onClick={() => setSignalType(s.id)} style={{
                  flex:1, padding:"10px 8px", borderRadius:10, cursor:"pointer", textAlign:"center",
                  background: signalType===s.id ? (s.id==="none" ? T.surfaceAlt : T.blueLight) : T.surfaceAlt,
                  border: `2px solid ${signalType===s.id ? (s.id==="none" ? T.borderStrong : T.blue) : T.border}`,
                }}>
                  <div style={{ fontSize:13, fontWeight:700, color: signalType===s.id && s.id!=="none" ? T.blue : T.text }}>{s.label}</div>
                  <div style={{ fontSize:11, color:T.textSub, marginTop:2 }}>{s.desc}</div>
                </button>
              ))}
            </div>
          </div>
      ...
      {result && (
        <Card>
          <SectionLabel>Schritt 3 – Auswertung</SectionLabel>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10, marginBottom:16 }}>
            {[
              {
                label: "Fenster offen",
                met: result.c1_window,
                detail: `${result.days} / ${trade.windowDays} Tage (${Math.round(result.windowPct*100)}% verstrichen)`,
                rule: "Weniger als 50% des Fensters muss noch übrig sein",
              },
              {
                label: "Potenzial vorhanden",
                met: result.c2_gain,
                detail: `${alreadyMoved || 0}% von Ø ${trade.avgGainPct}% gemacht (${Math.round(result.gainUsed*100)}%)`,
                rule: "Weniger als 50% des Ø-Gewinns darf bereits gelaufen sein",
              },
              {
                label: "Einstiegssignal",
                met: result.c3_signal,
                detail: signalType === "pullback" ? "Pullback auf Support ✓" : signalType === "breakout" ? "Ausbruch bestätigt ✓" : "Kein Signal vorhanden",
                rule: "Pullback auf MA/Support oder Ausbruch aus Konsolidierung",
              },
            ].map(c => (
              <div key={c.label} style={{
                padding:"14px", borderRadius:12, textAlign:"center",
                background: c.met ? T.greenLight : T.redLight,
                border:`1px solid ${c.met ? T.green+"55" : T.red+"33"}`,
              }}>
                <div style={{ fontSize:24, marginBottom:6 }}>{c.met ? "✅" : "❌"}</div>
                <div style={{ fontSize:13, fontWeight:700, color:c.met ? "#065f46" : T.red }}>{c.label}</div>
                <div style={{ fontSize:11, color:T.textSub, marginTop:4, lineHeight:1.4 }}>{c.detail}</div>
              </div>
            ))}
          </div>

          {/* Verdict */}
          <div style={{ background:result.bg, border:`2px solid ${result.color}44`, borderRadius:14, padding:"18px 20px", marginBottom:14 }}>
            <div style={{ fontSize:20, fontWeight:800, color:result.color, marginBottom:8 }}>{result.verdict}</div>
            <div style={{ fontSize:13, color:T.textSub, lineHeight:1.7 }}>{result.advice}</div>
          </div>
          ...
          {result.posSize !== "–" && (
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10 }}>
              {[
                { label:"Empfohlene Positionsgröße", val:result.posSize, color:T.blue,   bg:T.blueLight },
                { label:"Restpotenzial (ca.)",        val:`~${result.remaining.toFixed(1)}%`, color:T.green,  bg:T.greenLight },
                { label:"Noch verfügbare Tage",       val:`${result.daysLeft}d`,         color:T.yellow, bg:T.yellowLight },
              ].map(r => (
                <div key={r.label} style={{ background:r.bg, border:`1px solid ${r.color}33`, borderRadius:10, padding:"12px 14px", textAlign:"center" }}>
                  <div style={{ fontSize:22, fontWeight:800, color:r.color }}>{r.val}</div>
                  <div style={{ fontSize:11, color:T.textSub, marginTop:4 }}>{r.label}</div>
                </div>
              ))}
            </div>
          )}
...
export default function App() {
  ...
  const TABS = [["dashboard","Dashboard"],["checker","Späteinstieg"],["live","Live-Daten"],["rechner","Stop-Rechner"],["timeline","Kalender"]];
  ...
  return (
    <div style={{ minHeight:"100vh", background:T.bg, color:T.text, fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif", paddingBottom:60 }}>
      ...
      <div style={{ maxWidth:900, margin:"0 auto", padding:"22px 16px 0" }}>
        ...
        {/* ── SPÄTEINSTIEG-CHECKER TAB ── */}
        {tab === "checker" && <LateEntryChecker trades={TRADES} activeMonth={month} months={MONTHS} liveData={liveData} />}
